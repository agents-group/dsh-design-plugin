...
import { BRIDGE_SOURCE } from './bridge.ts'
import type { DesignModeProps } from './contract/slots.ts'
import { CHAT_WIDTH_DEFAULT, CHAT_WIDTH_MAX, CHAT_WIDTH_MIN, type DesignSelection } from './stores.ts'
import css from './DesignMode.module.css'

/** postMessage payload shape the overlay accepts from the preview iframe. */
...
export const DesignMode: FC<DesignModeProps> = ({ useStore, actions, citeSelection, removeCitation, useCitation }) => {
  ...
  const [selections, setSelections] = useState<Array<{ id: string; selection: DesignSelection }>>([])
  const [draftUrl, setDraftUrl] = useState(url)
  const [urlMenuOpen, setUrlMenuOpen] = useState(false)
  const chatDockRef = useRef<HTMLDivElement | null>(null)
  const dragRef = useRef<{ startX: number; startWidth: number; latestPct: number } | null>(null)
  ...
  useEffect(() => {
    ...
    const onMessage = (event: MessageEvent) => {
      const data = event.data as SelectionMessage | null
  ...
  const onDividerDown = useCallback((event: React.PointerEvent<HTMLDivElement>) => {
  ...
  return (
    <div ref={overlayRef} className={css.overlay} style={{ pointerEvents: 'none' }}>
      <div ref={chatDockRef} className={css.chatDock} style={{ width: `${chatWidth}%` }} aria-hidden="true" />
      <div
      ...
      />
      <section className={css.preview} style={{ pointerEvents: 'auto' }}>
        <header className={css.header}>
          <div className={css.urlWrap}>
            <input
              className={css.url}
              value={draftUrl}
              spellCheck={false}
            ...
            />
            {urlMenuOpen && urlHistory.length > 0 ? (
              <ul className={css.urlMenu}>
                {urlHistory.map(item => (
                  <li key={item}>
                    <button
                      type="button"
                      className={css.urlMenuItem}
                      onMouseDown={(event) => {
        ...
        {selections.length > 0 ? (
          <footer className={css.citation}>
            <span className={css.citationLabel}>引用 · {selections.length}</span>
            <span className={css.citationText} title={selections.map(item => item.selection.text).join('、')}>
              {selections.map(item => item.selection.text || item.selection.tagName).join('、')}
            </span>
            <button
              type="button"
              ...
              onClick={() => {
                void navigator.clipboard?.writeText(
                  selections.map(item => item.selection.text).filter(text => text !== '').join('\n'),
                )
              }}
            >
              复制
