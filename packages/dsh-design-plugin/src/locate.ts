...
function lineExcerpt(raw: string, start: number): string {
 ...
 */
export function locateSource(root: string, query: string): LocateResult {
  const result: LocateResult = { query, matches: [], truncated: false }
